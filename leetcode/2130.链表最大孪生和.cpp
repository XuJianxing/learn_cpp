...
public:
    int pairSum(ListNode* head) {
        // 先把链表存到数组里，然后按下标遍历所有孪生和，求最大值，就是空间占用较多
        ...
        int result = 0;
        std::size_t i = 0, j = values.size() - 1;
        while (i < j)
        {
    ...
    int pairSum2(ListNode* head) {
        // 快慢指针 + 反转链表
        // slow指向head，fast指向head.next
        // 随后slow每次移动1，fast每次移动2
        // 当fast到达最终节点时，slow指向链表前半部分的最后节点，slow.next指向链表后半部分起始节点
        // 然后后半部分反转链表，然后同时从头遍历两个链表，计算最大孪生和
        ListNode* slow = head;
