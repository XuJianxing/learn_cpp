 ...
 * 
 * 思路
 * 这道题的最优解法是用两个队列分别存天辉 R 和夜魇 D 参议员的下标，模拟投票过程：
 * 
 * 把每个参议员的初始下标（0..n-1）分别放入对应队列。
 * 每次比较两队队首的下标：
 * 下标更小的一方先出手，禁止对方（对方出队、不再回来）。
 * 胜利方自己的下标 +n 重新入队，表示进入下一轮（+n 保证它排在所有当前轮参议员之后）。
 * 循环直到某一队为空，另一方获胜。
 */
...
public:
    string predictPartyVictory(string senate) {
        ...
        while (!radiant.empty() && !dire.empty()) {
            ...
            // 序号靠前的一方先出手，禁止另一方；胜利方进入下一轮（下标 + n）
            if (r < d) {
                radiant.push(r + n);
            } else {
                dire.push(d + n);
